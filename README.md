...
| Evaluations | `WithinSubjectCV` (chronological folds with purge for regression; contiguous or stratified-shuffled trial folds for classification), `CrossSessionEvaluation` (leave one session out) | Cross-subject needs an explicit electrode aligner (roadmap M4); learning curves M2 |
| Pipelines | YAML reference set shipped in the wheel: `LogBandPower`/`HighGamma` x LDA/LogReg/Ridge, `Riemann TS + LogReg` (`[riemann]`), `ShallowFBCSPNet` (`[deep]`, `moecog.pipelines.deep`) | PACE zoo and grid search as pipeline steps still to do |
