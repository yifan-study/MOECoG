...
| Evaluations | `WithinSubjectCV` (chronological folds with purge for regression; contiguous or stratified-shuffled trial folds for classification), `CrossSessionEvaluation` (leave one session out), `LearningCurveEvaluation` (train on the first 10/25/50/100 % of a patient's trials, test on the fixed final 20 %) | Cross-subject needs an explicit electrode aligner (roadmap M4) |
| Pipelines | YAML reference set shipped in the wheel: `LogBandPower`/`HighGamma` x LDA/LogReg/Ridge, `Riemann TS + LogReg`, `Riemann HG TS + LogReg` (70-150 Hz band covariances), `Riemann Env TS + LogReg` (prototype covariances of log high-gamma envelopes) (`[riemann]`), `ShallowFBCSPNet` (`[deep]`, `moecog.pipelines.deep`) | PACE zoo and grid search as pipeline steps still to do |
