...
| Numbers | `results/reference_<task>.csv` for every task x pipeline, 5 chronological folds, three seeds where randomness enters; leaderboard regenerated | classical set done 2026-09-10 (motor 0.92, faces 0.68, imagery 0.66, gestures 0.49, fingerflex r 0.28, BCI III-1 within 0.82 / cross-session 0.52); ShallowFBCSPNet seed 0 done (motor 0.44, faces 0.76 = best pipeline, imagery 0.21, gestures 0.04); seeds 1-2 running on the Mac while Athene is down (BCI-34) |
...
| Learning curves | `LearningCurveEvaluation` (train on the first 10/25/50/100 % of a patient's trials, test on the fixed final 20 %; stratified fallback for block-ordered files), curves in `docs/analysis.md` | done 2026-09-11: one feature per channel (`HighGamma + LDA`) is the most data-efficient at 10 % of a motor session, the multi-band and band-limited Riemannian pipelines overtake it from 25 %; on BCI III-1 the low bands win at every budget |
| Better Riemannian baseline | `Riemann HG TS + LogReg`: tangent space on covariances of the 70-150 Hz band-passed epochs (`BandPassFilter`), the Riemannian counterpart of log band power | done 2026-09-11: best classical pipeline on motor_basic (kappa 0.94 vs 0.92 for `LogBandPower + LDA`, 0.70 for raw-epoch covariances) and faces_basic (0.74 vs 0.68), behind where high gamma is not the band (gestures 0.39 vs 0.52, BCI III-1 0.62 vs 0.82); per-patient means in `docs/analysis.md`; envelope-prototype covariances (`ERPCovariances`) trail everything and stay under `examples/pipelines/` as a negative result |
...
| Session drift | label-free per-session alignment as an evaluation option (`moecog.alignment`: Euclidean alignment, Riemannian re-centering, raw z-score) and per-session feature standardisation as a pipeline step (`BatchStandardizer`), evaluated on BCI III-1 cross-session; the literature says alignment is worth 3-10 points and no single method wins (`docs/best_practices.md`) | done for BCI III-1 2026-09-11: Riemannian re-centering lifts the Riemannian pipeline from 0.55 to 0.79 across the sessions, per-session feature standardisation lifts band power from 0.36 to 0.75 (within-session 0.80 to 0.82); Euclidean alignment hurts everything there. Needs a second multi-session dataset family to become a population claim |
...
- 2026-09-11 (evening): M3 chunk 1 (session drift) done on BCI III-1 with two label-free fixes, one per feature family. Second-family audit: every OpenNeuro copy on the Mac is a single-patient smoke download, so the family is a fresh download; recommended Peterson move/rest (12 patients, several days each, HTNet numbers to compare, loader in place, 10.7 GB) on BCI-38, with ds003374 / dandi-001616 / ds004770 as cheap add-ons and the large CC0 speech sets (ds005007, ds006233, ds006914) for M5.
- 2026-09-11: M2's last two chunks landed (learning curves, band-limited Riemannian baseline) while Athene stayed unreachable from off campus (FAU closed direct SSH to the login node; OnDemand or the Duo bastion are the doors now). Nothing in M2 needed the cluster: the Miller tier fits a laptop. The deep seeds finish on the Mac; M3 starts with drift alignment on BCI III-1, where the cross-session gap is largest.
