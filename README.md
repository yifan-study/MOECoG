...
MOECoG follows the MOABB model: a BSD-licensed package, datasets and pipelines added one pull request at a time,
results regenerated by scripts. `CONTRIBUTING.md` says how to add a dataset, a paradigm, a pipeline (YAML in
`moecog/pipelines/configs/`) or a result; `ROADMAP.md` lists the milestones in week-sized chunks with their definitions of done;
`docs/moabb_lessons.md` records what MOABB did and `docs/moabb_review.md` decides, component by component, what
we copy, adapt or improve. One-call benchmark:
...
| Evaluations | `WithinSubjectCV` (chronological folds with purge for regression; contiguous or stratified-shuffled trial folds for classification), `CrossSessionEvaluation` (leave one session out), `LearningCurveEvaluation` (train on the first 10/25/50/100 % of a patient's trials, test on the fixed final 20 %); every evaluation takes `alignment="ea"` / `"recenter"` / `"zscore"` for label-free per-session alignment (`moecog.alignment`) | Cross-subject needs an explicit electrode aligner (roadmap M4) |
| Pipelines | YAML reference set shipped in the wheel: `LogBandPower`/`HighGamma` x LDA/LogReg/Ridge, `Riemann TS + LogReg`, `Riemann HG TS + LogReg` (70-150 Hz band covariances; `[riemann]`), `ShallowFBCSPNet` (`[deep]`, `moecog.pipelines.deep`) | PACE zoo and grid search as pipeline steps still to do |
| Preprocessing | `CommonAverageReference`, `NotchFilter`, `HilbertEnvelope`, `BandPassFilter`, `chang_high_gamma()`; paradigms take `raw_steps=[...]` | Survey of the field's pipelines in `docs/preprocessing_catalog.md` |
...
| Results | `results/reference_<task>.csv` (`ResultsStore`) + `scripts/build_leaderboard.py` -> `docs/leaderboard.md`, `scripts/build_analysis.py` -> `docs/analysis.md` | reference benchmark: motor_basic kappa 0.92 (19 pts), faces 0.76 (ShallowFBCSPNet) / 0.68 (band power), imagery 0.66, gestures 0.52, fingerflex r 0.28, BCI III-1 within 0.82 / cross-session 0.52; findings in `docs/baselines.md` |
